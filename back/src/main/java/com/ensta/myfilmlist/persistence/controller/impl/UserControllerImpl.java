...
public class UserControllerImpl implements UserController {
    ...
    @Override
    @GetMapping("/")
    public ResponseEntity<UserDTO> getUserByUsername(@RequestParam String username) throws ControllerException {
        try {
            UserDTO userDTO = UserMapper.convertUserToUserDTO(myFilmsService.findByUsername(username));
            if (userDTO == null) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);
            }
            return ResponseEntity.status(HttpStatus.OK).body(userDTO);
